...
### 13. Đổi mô hình lịch đặt sân thành đơn đặt sân + chi tiết đặt sân
- Bảng `lịch đặt sân` được đổi thành `đơn đặt sân` để đại diện cho một đơn đặt tổng.
- `đơn đặt sân` lưu thông tin cấp đơn, gồm: `id_đơn_đặt_sân`, `user_id`, `thời_gian_tạo`, `trạng_thái`, `tổng_tiền`, `trạng_thái_thanh_toán`, `ghi_chú`.
- Mỗi `đơn đặt sân` có thể bao gồm nhiều `chi tiết đặt sân`.
- Bổ sung bảng `chi tiết đặt sân`, gồm: `id_chi_tiết_đặt_sân`, `id_đơn_đặt_sân` (FK), `id_sân` (FK), `start_time`, `end_time`.
- Quan hệ: một `đơn đặt sân` có nhiều `chi tiết đặt sân`; mỗi `chi tiết đặt sân` chỉ thuộc về một `đơn đặt sân`.
- Mỗi `chi tiết đặt sân` đại diện cho một sân trong một khung giờ cụ thể.
- Thiết kế này hỗ trợ cả đặt sân đơn và đặt combo nhiều sân/nhiều khung giờ trong cùng một đơn, ví dụ đặt sân cho giải đấu hoặc sự kiện thể thao.
- Khi kiểm tra trùng lịch/overlap, hệ thống phải kiểm tra trên bảng `chi tiết đặt sân`, không chỉ trên `đơn đặt sân`.
- Thanh toán và tổng tiền xử lý ở cấp `đơn đặt sân`; check-in/no-show/hoàn thành có thể xử lý ở cấp từng `chi tiết đặt sân` nếu nghiệp vụ cần theo dõi riêng từng sân/khung giờ.